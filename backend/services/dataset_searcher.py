...
class ArbovirusDataSearcher(ABC):
    ...
    @abstractmethod
    def get_cases_dates(self, start_date: int, end_date: int, city_code: int) -> list[dengue.DengueCase]:
        pass   
...
class DengueDataSearcher(ArbovirusDataSearcher):
    ...
    def _get_city_index(self, date: datetime, city_code: int) -> engine.IndexRegister:
    ...
    def _get_cases_from_city(self, date: datetime, city_code: int) -> list[dengue.DengueCase]:
        index = self._get_city_index(date, city_code)
        if index is None: return None
    ...
    def _get_cases_after_date_in_month(self, date: datetime, city_code: int) -> list[dengue.DengueCase]:
    ...
    def _get_cases_before_date_in_month(self, date: datetime, city_code: int) -> list[dengue.DengueCase]:
        cases = self._get_cases_from_city(date, city_code)
        if cases is None: return []
    ...
    def _get_cases_entire_month(self, date: datetime, city_code: int) -> list[dengue.DengueCase]:
        cases = self._get_cases_from_city(date, city_code)
        if cases is None: return []
